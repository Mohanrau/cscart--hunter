...
class RusSdek
{
    ...
    public static function cityId($location)
    {
        ...
        if (!empty($country) && !empty($state) && !empty($location['city'])) {
            ...
            $data_cities = db_get_fields("SELECT c.sdek_city_code FROM ?:rus_city_descriptions as d LEFT JOIN ?:rus_cities as c ON c.city_id = d.city_id ?p AND d.city = ?s AND c.sdek_city_code <> '' AND d.lang_code = ?s", $condition, $city, CART_LANGUAGE);

            if (count($data_cities) < 1) {
                if (AREA != 'C') {
